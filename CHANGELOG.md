# Changelog

## v0.1.0 - Initial Fabricum release

2026-09-12

### Added

- Added a local browser GUI for selecting image sources and editing square and
  4:3 crops.
- Added CLI mode for path-driven processing and export automation.
- Added PNG, WebP, and AVIF output with quality and lossless options.
- Added a standalone Go executable with embedded frontend assets and local-only
  processing.
