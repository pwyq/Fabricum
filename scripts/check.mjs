...
const files = goFiles('front-end').concat(goFiles('back-end'))
...
run('go',['build','./...'])
if (!readFileSync(resolve(root,'back-end/internal/editor/config.go'),'utf8').includes('Version = "'+releaseVersion+'"')) throw new Error('Go and release versions differ')
